...
extern color    labelColor                    = MediumGreen;  // Color of label, off-hours  [DarkGreen]
extern color    clockColor                    = MediumGreen; // Color of clock, off-hours  [MediumBlue]
...
int init()
  {
   ...
   myTraceLogger.log( LOG_OFF, sLogPrefix + "Starting Logging...", LOG_OFF);  //LOGGING
...
int start()
  {
   ...
   string UTCs = xTimeToString( UTC );
   string sydneys = xTimeToString(sydney);
   string locals = xTimeToString( localTime  );
   string londons = xTimeToString( london  );
   string tokyos = xTimeToString( tokyo  );
   string aucklands = xTimeToString( auckland  );
   string moscows = xTimeToString( moscow  );
   string berlins = xTimeToString( berlin  );
   string seattles = xTimeToString( seattle  );
   string newyorks = xTimeToString( newyork  );
   string brokers = xTimeToString( TimeCurrent() );
   ...
   if ( ShowLocal ) {
      myTraceLogger.log( LOG_DEBUG, StringConcatenate( sLogPrefix, "Local: ", locals), kFunctionLoggingLevel );   //LOGGING
   ...
   if(ShowNewYork)
   {
   	...
   	{
         myTraceLogger.log( LOG_DEBUG, StringConcatenate( sLogPrefix, "* New York: ", newyorks), kFunctionLoggingLevel );   //LOGGING
   ...
   if(ShowLondon)
   {
   	...
   	{
         myTraceLogger.log( LOG_DEBUG, StringConcatenate( sLogPrefix, "London: ", londons), kFunctionLoggingLevel );   //LOGGING
   ...
   if(ShowTokyo)
   {
   	...
   	{
         myTraceLogger.log( LOG_DEBUG, StringConcatenate( sLogPrefix, "Tokyo: ", tokyos), kFunctionLoggingLevel );   //LOGGING
   ...
   if(ShowSydney)
   {
   	...
   	{
         myTraceLogger.log( LOG_DEBUG, StringConcatenate( sLogPrefix, "* Sydney: ", sydneys), kFunctionLoggingLevel );   //LOGGING
...
string xTimeToString( datetime when ) {
   string timeStr;
